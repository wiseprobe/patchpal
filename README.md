...
The `/status` command shows cumulative token usage, which helps you understand API costs:

- **Cumulative input tokens**: Total tokens sent to the LLM across all calls
  - This is where costs accumulate rapidly due to context growth
  - Each LLM call resends the entire conversation history
  - Example: 50 operations with growing context = millions of input tokens
...
To estimate costs: multiply tokens by your model's pricing (see your provider's documentation).
...
- **Session Statistics** section shows:
  - Total LLM calls made
  - Cumulative input tokens (where costs accumulate)
  - Cumulative output tokens
  - Total tokens for the session
- Check periodically during long sessions to track costs
- Use this data to estimate API charges (multiply by your model's pricing)
