...
### fixed:
- Ensure Autopilot mode loads custom tools (#56)
...
- **`code_structure` tool**: Added tree-sitter AST parsing tool to analyze code structure without reading full files. Shows classes, methods, functions with line numbers and signatures. Supports 40+ languages (Python, JavaScript, TypeScript, Go, Rust, Java, C/C++, etc.). Achieves 95% token savings vs read_file for large code files (10K → 500 tokens). Most effective for targeted exploration of specific code sections. (#46)
